...
그 예측이 실제로 맞는지를 보여주는 수치가 없었습니다. 이상거래 탐지는 정답지 대비 성능을
제시할 수 있었지만(재측정 결과 탐지율 67.7% / 정상거래 오탐률 4.9%), 정작 핵심 기능은
"그럴듯해 보인다" 이상의 근거가 없었던 셈입니다.
...
def forecast_accuracy(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "limitations": (
            ...
            "특히 정산 '날짜'는 시드 데이터가 고정 주기로 생성되어 오차가 거의 0으로 나오는데, "
            "이는 예측력이 완벽하다는 뜻이 아니라 구현이 의도대로 동작한다는 확인에 가깝습니다. "
            "실제 예측력을 보여주는 지표는 변동성이 들어있는 '금액' 오차와 반복 지출 발생일 오차이며, "
            "확정적인 수치는 실제 정산 데이터를 연동한 뒤 재측정해야 합니다."
        ),
    }
